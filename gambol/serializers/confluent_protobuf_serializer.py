...
class ConfluentProtobufSerializer(Serializer):
    ...
    def serialize(self, value: Message, serialization_context: SerializationContext) -> SerializedMessage:
        if not self.schema_registry_client:
            raise SerializationError("a schema registry client is required when serializing a message")
        
        confluent_serialization_context = ConfluentSerializationContext(
            topic=serialization_context.destination.topic if serialization_context.destination else None,
    ...
    def deserialize(self, value: bytes, model: Type[ProtobufMessage_T]) -> ProtobufMessage_T:
        try:
