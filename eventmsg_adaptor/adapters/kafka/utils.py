...
from eventmsg_adaptor.utils import parse_event_name_and_version_from_protobuf_message, normalise_destination
...
def parse_event_headers(event_body: EventBody, message: ConsumerRecord) -> EventHeaders:
    ...
    if isinstance(event_body, Message):
        event_name, version = parse_event_name_and_version_from_protobuf_message(event_body)
        
        if not hasattr(event_body, "event_fields"):
            raise Exception("The message does not appear to be a sanctumlabs-proto type message")
